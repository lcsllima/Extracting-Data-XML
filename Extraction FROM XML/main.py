...
conexao = sqlite3.connect('musicas.sqlite')
...
CREATE TABLE Track (
    ...
    titulo TEXT UNIQUE,
    album_id INTEGER, 
    tempo INTEGER, avaliação INTEGER, count INTEGER
);
''')
...
for entrada in filtrado:
    ...
    jarvees.execute('''INSERT OR REPLACE INTO TRACK (titulo, album_id, tempo, avaliação, count) VALUES (?, ?, ?, ?, ?)''',
                    (nome, album_id, tempo, avaliação, count))
